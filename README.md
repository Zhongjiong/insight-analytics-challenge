...
For feature 3, there are something needed to discussed further. 
If overlapping is allowed, it will be really easy to get them.
If overlapping is not allowed, we should consider the distance between each intervals. Here I assume that overlapping is not allowed as it makes more sense.
There may be a few busy intervals close to each other and we could not choose all of them. If we pick up the busiest time interval, there may be two busy time intervals which is very close to it without overlapping from each other. And we could not choose them because we have already picked up that busiest interval.

So here I just picked up the busiest time interval and then picked up next busiest time interval which is not within 60-minute of the intervals picked up before.
